    ...
    1: [
      ...
      function(_dereq_, module, exports) {
        ...
        function Neo4jD3(_selector, _options) {
          var container, info, node, nodes, relationship,
            relationshipOutline, relationshipOverlay,
            relationshipText, relationships, selector,
            simulation, svg, svgNodes, svgRelationships,
            svgScale, svgTranslate,
            classes2colors = {},
            numClasses = 0,
            options = {
              arrowSize: 4,
            ...
            if (!value) {
              elem.style('background-color', function(d) {
                return options.nodeOutlineFillColor ?
                  ...
                  (isNode ? class2color(property) : defaultColor())
              }).style('border-color', function(d) {
                return options.nodeOutlineFillColor ?
                  class2darkenColor(options.nodeOutlineFillColor) :
                  (isNode ? class2darkenColor(property) : defaultDarkenColor())
          ...
          // LYL232 adjusted: show node name
          function appendNodeName(node) {
...
//            color = options.colors[Math.min(numClasses, options.colors.length - 1)];
          ...
          function image(d) {
            var i, imagesForLabel, img, imgLevel, label, labelPropertyValue,
              property, value
          ...
          function rotation(source, target) {
            return Math.atan2(target.y - source.y, target.x - source.x) * 180 /
              Math.PI
          }
          ...
          */
              ...
              outline.attr('d', function(d) {
                var center = {x: 0, y: 0},
                  ...
                  rotatedPointB2 = rotatePoint(center, {
                    x: d.target.x - d.source.x - (options.nodeRadius + 1) *
                      u.x - n.x - u.x * options.arrowSize,
                    y: d.target.y - d.source.y - (options.nodeRadius + 1) *
            ...
            relationshipOverlay.attr('d', function(d) {
              var center = {x: 0, y: 0},
                ...
                rotatedPointD = rotatePoint(center,
                  {x: 0 + n.x - n1.x, y: 0 + n.y - n1.y}, angle)
          ...
          function unitaryVector(source, target, newLength) {
            var length = Math.sqrt(Math.pow(target.x - source.x, 2) +
              Math.pow(target.y - source.y, 2)) / Math.sqrt(newLength || 1)
          ...
          function updateInfo(d) {
            ...
            Object.keys(d.properties).forEach(function(property) {
              appendInfoElementProperty('property', property,
                JSON.stringify(d.properties[property]))
            })
          }
          ...
          function updateNodes(n) {
            ...
            node = appendNodeToGraph().merge(node)
          }
          ...
          function updateRelationships(r) {
            ...
            relationshipOutline = relationshipEnter.outline.merge(
              relationshipOutline)

            relationshipOverlay = svg.selectAll('.relationship .overlay')
            relationshipOverlay = relationshipEnter.overlay.merge(
              relationshipOverlay)
          ...
          function zoomFit(transitionDuration) {
            ...
            svg.attr('transform',
              'translate(' + svgTranslate[0] + ', ' + svgTranslate[1] +
              ') scale(' + svgScale + ')')
